...
record_results_txt = ['PSNR_SSIM_Results_', degradation,'_model_',methods{1}, '.txt'];
    ...
    for idx_set = 1:num_set
        ...
        for scale = scale_all
            ...
            for idx_ext = 1:length(ext)
                filepaths = cat(1, filepaths, dir(fullfile([methods{idx_method}, '_x' ,num2str(scale), '/results/' ,dataset{idx_set}] , ['*_x', num2str(scale), '_HR', ext{idx_ext}])));
                sr_filepaths = cat(1, sr_filepaths, dir(fullfile([methods{idx_method}, '_x' ,num2str(scale), '/results/', dataset{idx_set}] , ['*_x', num2str(scale), '_', methods{idx_method}, ext{idx_ext}])));
            end
            ...
            for idx_im = 1:length(filepaths)
                ...
                name_SR = sr_filepaths(idx_im).name;

                 im_HR = imread(fullfile([methods{idx_method}, '_x' ,num2str(scale), '/results/' ,dataset{idx_set}],  name_HR));
                 im_SR = imread(fullfile([methods{idx_method}, '_x' ,num2str(scale), '/results/' ,dataset{idx_set}],  name_SR));
                ...
                fprintf(results, 'x%d %d %s: PSNR= %.2f SSIM= %.4f\n', scale, idx_im, name_SR, PSNR_all(idx_im), SSIM_all(idx_im));
                fprintf('x%d %d %s: PSNR= %.2f SSIM= %.4f\n', scale, idx_im, name_SR, PSNR_all(idx_im), SSIM_all(idx_im));
            end
            fprintf(results, '--------Mean--------\n');
            fprintf('--------Mean--------\n');
            fprintf(results, 'x%d: PSNR= %.2f SSIM= %.4f\n\n', scale, mean(PSNR_all), mean(SSIM_all));
